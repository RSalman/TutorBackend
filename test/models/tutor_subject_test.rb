...
class TutorSubjectTest < ActiveSupport::TestCase
  def setup
    ...
    @user = User.new(first_name: 'tom', last_name: 'brady', email: 'tb@gmail.com', password: 'securepass',
                     phone_number: '6135265547')
  end

  test 'valid subject' do
    subject = @user.tutor_subjects.new(rate: 1, course_id: @course.id)
  ...
  test 'valid subject no course' do
    subject = @user.tutor_subjects.new(rate: 1)
  ...
  test 'invalid subject no rating' do
    subject = @user.tutor_subjects.new(course_id: @course.id)
